...
def execute_tools(tool_calls: List) -> List[dict]:
    ...
    for tool_call in tool_calls:
        try:
            ...
            # 1. {"command": "git status"} - standard
            # 2. {"cmd": ["git status"]} - list format (qwen3)
            # 3. {"cmd": "git status"} - string format
            if 'command' in args:
                command = args.get('command', '')
            elif 'cmd' in args:
                cmd_val = args.get('cmd', '')
                if isinstance(cmd_val, list):
                    command = ' '.join(str(c) for c in cmd_val)
                else:
                    command = str(cmd_val)
            else:
                command = ''
            
            # Strip leading "bash -lc" or "bash -c" prefix if model sent shell wrapper
            # e.g., "bash -lc dir" -> "dir" on Windows, "bash -c ls" -> "ls" on Unix
            command = command.strip()
            if command.lower().startswith('bash '):
                # Remove "bash -lc" or "bash -c" prefix, keep only the actual command
                remainder = command[5:].strip()  # after "bash "
                for prefix in ('-lc', '-c'):
                    if remainder.lower().startswith(prefix):
                        command = remainder[len(prefix):].strip()
                        break
                else:
                    # No known prefix found, take the whole remainder as command
                    command = remainder
